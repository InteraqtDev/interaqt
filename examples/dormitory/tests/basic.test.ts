...
describe('Basic Functionality', () => {
  ...
  test('User entity creation via CreateUser interaction', async () => {
    ...
    expect(user.currentScore).toBe(0) // No score events yet, so summation is 0
  })
  ...
  test('Dormitory entity creation via CreateDormitory interaction', async () => {
    ...
    expect(dormitory.occupiedBeds).toBe(0) // No residents yet, so count is 0
  ...
  test('AuditLog.id has correct auto-generated value (_owner)', async () => {
    /**
     * Test Plan for: AuditLog.id (_owner)
     * This tests that AuditLog.id is properly auto-generated when AuditLog is created by system
     * Dependencies: AuditLog entity, system interactions
     * Steps: 1) Execute a system interaction that creates AuditLog 2) Verify id is auto-generated and set
     * Business Logic: _owner properties are controlled by entity creation - id is auto-generated by the framework
     */
    
    // Create a user interaction to trigger AuditLog creation
    const userResult = await controller.callInteraction('CreateUser', {
      user: { id: 'admin' },
      payload: {
        username: 'auditidtestuser',
        email: 'auditid@example.com',
        password: 'password123',
        fullName: 'Audit ID Test User',
        role: 'student'
      }
    })
    ...
    // Query the created AuditLog entries to verify id is properly set
    const auditLogs = await system.storage.find('AuditLog',
      MatchExp.atom({ key: 'actionType', value: ['=', 'CreateUser'] }),
    ...
    // Verify id is properly generated
    expect(auditLog.id).toBeDefined()
    expect(typeof auditLog.id).toBe('string')
    expect(auditLog.id.length).toBeGreaterThan(0)
    expect(auditLog.actionType).toBe('CreateUser')
    expect(auditLog.timestamp).toBeDefined()
    expect(auditLog.details).toBeDefined()
    
    // Verify details contain expected structure
    const details = JSON.parse(auditLog.details)
    expect(details.interaction).toBe('CreateUser')
    expect(details.payload.username).toBe('auditidtestuser')
  })

  test('AuditLog.actionType set by owner computation (_owner)', async () => {
    /**
     * Test Plan for: AuditLog.actionType (_owner)
     * This tests that AuditLog.actionType is properly set when AuditLog is created by system
     * Dependencies: AuditLog entity, system interactions
     * Steps: 1) Execute multiple different interactions 2) Verify actionType is set correctly for each
     * Business Logic: _owner properties are controlled by entity creation - actionType is set based on triggering interaction type
     */
    
    // Create a user to trigger AuditLog creation
    const userResult = await controller.callInteraction('CreateUser', {
      user: { id: 'admin' },
      payload: {
        username: 'actiontypetestuser',
        email: 'actiontype@example.com',
        password: 'password123',
        fullName: 'Action Type Test User',
        role: 'student'
      }
    })
    ...
    // Create a dormitory to trigger another AuditLog
    const dormitoryResult = await controller.callInteraction('CreateDormitory', {
      user: { id: 'admin' },
      payload: {
        name: 'Action Type Test Dormitory',
        bedCount: 4,
        building: 'A',
        floor: 1
      }
    ...
    // Query CreateUser AuditLog
    const createUserLogs = await system.storage.find('AuditLog',
      MatchExp.atom({ key: 'actionType', value: ['=', 'CreateUser'] }),
      { limit: 1, orderBy: { timestamp: 'desc' } },
      ['id', 'actionType', 'timestamp', 'details']
    )
    
    expect(createUserLogs.length).toBeGreaterThan(0)
    const createUserLog = createUserLogs[0]
    
    // Verify actionType is properly set for CreateUser
    expect(createUserLog.actionType).toBe('CreateUser')
    expect(createUserLog.id).toBeDefined()
    expect(createUserLog.timestamp).toBeDefined()
    
    const createUserDetails = JSON.parse(createUserLog.details)
    expect(createUserDetails.interaction).toBe('CreateUser')
    expect(createUserDetails.payload.username).toBe('actiontypetestuser')
    
    // Query CreateDormitory AuditLog
    const createDormitoryLogs = await system.storage.find('AuditLog',
      MatchExp.atom({ key: 'actionType', value: ['=', 'CreateDormitory'] }),
      { limit: 1, orderBy: { timestamp: 'desc' } },
      ['id', 'actionType', 'timestamp', 'details']
    )
    
    expect(createDormitoryLogs.length).toBeGreaterThan(0)
    const createDormitoryLog = createDormitoryLogs[0]
    
    // Verify actionType is properly set for CreateDormitory
    expect(createDormitoryLog.actionType).toBe('CreateDormitory')
    expect(createDormitoryLog.id).toBeDefined()
    expect(createDormitoryLog.timestamp).toBeDefined()
    
    const createDormitoryDetails = JSON.parse(createDormitoryLog.details)
    expect(createDormitoryDetails.interaction).toBe('CreateDormitory')
    expect(createDormitoryDetails.payload.name).toBe('Action Type Test Dormitory')
  })

  test('AuditLog.timestamp set by owner computation (_owner)', async () => {
    /**
     * Test Plan for: AuditLog.timestamp (_owner)
     * This tests that AuditLog.timestamp is properly set when AuditLog is created by system
     * Dependencies: AuditLog entity, system interactions
     * Steps: 1) Execute a system interaction that creates AuditLog 2) Verify timestamp is set to current time
     * Business Logic: _owner properties are controlled by entity creation - timestamp is set to current timestamp when audit log is created
     */
    
    // Record time before creating interaction
    const beforeTime = Math.floor(Date.now() / 1000)
    ...
    // Record time after creating interaction
    const afterTime = Math.floor(Date.now() / 1000)

    expect(userResult.error).toBeUndefined()
    
    // Query the most recent AuditLog entry
    const auditLogs = await system.storage.find('AuditLog',
      MatchExp.atom({ key: 'actionType', value: ['=', 'CreateUser'] }),
      { limit: 1, orderBy: { timestamp: 'desc' } },
      ['id', 'actionType', 'timestamp', 'details']
    ...
    // Verify timestamp is properly set and within reasonable time window
    expect(auditLog.timestamp).toBeDefined()
    expect(typeof auditLog.timestamp).toBe('number')
    expect(auditLog.timestamp).toBeGreaterThanOrEqual(beforeTime)
    expect(auditLog.timestamp).toBeLessThanOrEqual(afterTime)
    ...
    const details = JSON.parse(auditLog.details)
    expect(details.interaction).toBe('CreateUser')
    expect(details.payload.username).toBe('timestamptestuser')
  })

  test('AuditLog.details set by owner computation (_owner)', async () => {
    /**
     * Test Plan for: AuditLog.details (_owner)
     * This tests that AuditLog.details is properly set when AuditLog is created by system
     * Dependencies: AuditLog entity, system interactions
     * Steps: 1) Execute a system interaction that creates AuditLog 2) Verify details contains serialized information about the action and its context
     * Business Logic: _owner properties are controlled by entity creation - details is serialized information about the action and its context
     */
    ...
    // Query the most recent CreateUser AuditLog entry
    const auditLogs = await system.storage.find('AuditLog',
      MatchExp.atom({ key: 'actionType', value: ['=', 'CreateUser'] }),
      { limit: 1, orderBy: { timestamp: 'desc' } },
      ['id', 'actionType', 'timestamp', 'details']
    ...
    // Verify details is properly set
    expect(auditLog.details).toBeDefined()
    expect(typeof auditLog.details).toBe('string')
    
    // Parse and verify details structure
    let details
    expect(() => {
      details = JSON.parse(auditLog.details)
    }).not.toThrow()
    
    // Verify details contains expected structure
    expect(details).toBeDefined()
    expect(details.userId).toBe('admin')
    expect(details.interaction).toBe('CreateUser')
    expect(details.payload).toBeDefined()
    expect(details.payload.username).toBe('detailstestuser')
    expect(details.payload.email).toBe('details@example.com')
    expect(details.payload.fullName).toBe('Details Test User')
    expect(details.payload.role).toBe('admin')
    expect(details.payload.password).toBe('password123')
    
    // Verify other audit log properties are correctly set
    expect(auditLog.id).toBeDefined()
    expect(auditLog.actionType).toBe('CreateUser')
    expect(auditLog.timestamp).toBeDefined()
  })

  test('User.currentScore Summation computation', async () => {
    /**
     * Test Plan for: User.currentScore
     * Dependencies: User entity, UserScoringRelation, ScoreEvent entity, ApplyScoreDeduction interaction
     * Steps: 1) Create user 2) Apply multiple score deductions 3) Verify currentScore sums all ScoreEvent amounts
     * Business Logic: Summation of all ScoreEvent amounts linked to this user
     */
    ...
    const userResult = await controller.callInteraction('CreateUser', {
      user: { id: 'admin' },
      payload: {
        username: 'scoresummationuser',
        email: 'scoresummation@example.com',
        password: 'password123',
        fullName: 'Score Summation User',
        role: 'student'
      }
    })
    ...
    // Get the created user ID
    const users = await system.storage.find('User',
      MatchExp.atom({ key: 'username', value: ['=', 'scoresummationuser'] }),
      undefined,
      ['id', 'username', 'currentScore']
    )
    const userId = users[0].id

    // Initial score should be 0 (no score events yet)
    expect(users[0].currentScore).toBe(0)

    // Apply first score deduction: -25 points
    const scoreResult1 = await controller.callInteraction('ApplyScoreDeduction', {
      user: { id: 'admin' },
      payload: {
        userId: userId,
        deductionAmount: 25,
    ...
    expect(scoreResult1.error).toBeUndefined()

    // Apply second score deduction: -15 points
    const scoreResult2 = await controller.callInteraction('ApplyScoreDeduction', {
      user: { id: 'admin' },
      payload: {
        userId: userId,
        deductionAmount: 15,
    ...
    expect(scoreResult2.error).toBeUndefined()

    // Apply third score deduction: -10 points
    const scoreResult3 = await controller.callInteraction('ApplyScoreDeduction', {
      user: { id: 'admin' },
      payload: {
        userId: userId,
        deductionAmount: 10,
    ...
    expect(scoreResult3.error).toBeUndefined()

    // Check currentScore after all deductions
    ...
    // Should sum all score events: -25 + -15 + -10 = -50
    expect(updatedUser.currentScore).toBe(-50)
    expect(updatedUser.username).toBe('scoresummationuser')

    // Verify the underlying ScoreEvents were created correctly
    const scoreEvents = await system.storage.find('ScoreEvent',
      MatchExp.atom({ key: 'reason', value: ['in', ['Late submission', 'Missed class', 'Dress code violation']] }),
      undefined,
      ['id', 'amount', 'reason', 'category', 'timestamp']
    )
    ...
    const sortedScoreEvents = scoreEvents.sort((a, b) => a.timestamp - b.timestamp)
    expect(sortedScoreEvents[0].amount).toBe(-25)
    expect(sortedScoreEvents[0].reason).toBe('Late submission')
    expect(sortedScoreEvents[1].amount).toBe(-15)
    expect(sortedScoreEvents[1].reason).toBe('Missed class')
    expect(sortedScoreEvents[2].amount).toBe(-10)
    expect(sortedScoreEvents[2].reason).toBe('Dress code violation')

    // Verify the UserScoringRelations exist
    const { UserScoringRelation } = await import('../backend')
    const userScoringRelations = await system.storage.find(UserScoringRelation.name,
      MatchExp.atom({ key: 'source.id', value: ['=', userId] }),
      undefined,
      [
        'id',
    ...
    expect(userScoringRelations.length).toBe(3)
    expect(userScoringRelations.every(rel => rel.source.id === userId)).toBe(true)
    // Sort in ascending order: [-25, -15, -10] becomes [-25, -15, -10]
    expect(userScoringRelations.map(rel => rel.target.amount).sort((a, b) => a - b)).toEqual([-25, -15, -10])
  })

  test('Dormitory.occupiedBeds Count computation', async () => {
    /**
     * Test Plan for: Dormitory.occupiedBeds
     * Dependencies: Dormitory entity, BedAssignmentRelation, User entity, AssignUserToBed interaction, RemoveUserFromDormitory interaction
     * Steps: 1) Create dormitory and users 2) Assign users to beds 3) Verify occupiedBeds count increases 4) Remove user from dormitory 5) Verify occupiedBeds count decreases
     * Business Logic: Count of active bed assignments for this dormitory
     */
    ...
    // Get the created dormitory ID
    const dormitories = await system.storage.find('Dormitory',
      MatchExp.atom({ key: 'name', value: ['=', 'Occupancy Test Dormitory'] }),
      undefined,
      ['id', 'name', 'bedCount', 'occupiedBeds']
    )
    const dormitoryId = dormitories[0].id

    // Initially, no beds should be occupied
    expect(dormitories[0].occupiedBeds).toBe(0)

    // Create first user and assign to bed
    const user1Result = await controller.callInteraction('CreateUser', {
      user: { id: 'admin' },
      payload: {
        username: 'occupancyuser1',
        email: 'occupancy1@example.com',
        password: 'password123',
        fullName: 'Occupancy User 1',
        role: 'student'
      }
    })
    ...
    const user1 = await system.storage.findOne('User',
      MatchExp.atom({ key: 'username', value: ['=', 'occupancyuser1'] }),
      undefined,
      ['id', 'username']
    )

    // Assign user 1 to bed 1
    const assign1Result = await controller.callInteraction('AssignUserToBed', {
      user: { id: 'admin' },
      payload: {
        userId: user1.id,
        dormitoryId: dormitoryId,
    ...
    expect(assign1Result.error).toBeUndefined()

    // Check occupiedBeds after first assignment
    const dormitoryAfter1 = await system.storage.findOne('Dormitory',
      MatchExp.atom({ key: 'id', value: ['=', dormitoryId] }),
      undefined,
      ['id', 'name', 'occupiedBeds']
    )

    expect(dormitoryAfter1.occupiedBeds).toBe(1)

    // Create second user and assign to bed
    const user2Result = await controller.callInteraction('CreateUser', {
      user: { id: 'admin' },
      payload: {
        username: 'occupancyuser2',
        email: 'occupancy2@example.com',
        password: 'password123',
        fullName: 'Occupancy User 2',
        role: 'student'
      }
    })
    ...
    const user2 = await system.storage.findOne('User',
      MatchExp.atom({ key: 'username', value: ['=', 'occupancyuser2'] }),
      undefined,
      ['id', 'username']
    )

    // Assign user 2 to bed 2
    const assign2Result = await controller.callInteraction('AssignUserToBed', {
      user: { id: 'admin' },
      payload: {
        userId: user2.id,
        dormitoryId: dormitoryId,
    ...
    expect(assign2Result.error).toBeUndefined()

    // Check occupiedBeds after second assignment
    const dormitoryAfter2 = await system.storage.findOne('Dormitory',
      MatchExp.atom({ key: 'id', value: ['=', dormitoryId] }),
      undefined,
      ['id', 'name', 'occupiedBeds']
    )

    expect(dormitoryAfter2.occupiedBeds).toBe(2)

    // Remove user 1 from dormitory
    const removeResult = await controller.callInteraction('RemoveUserFromDormitory', {
      user: { id: 'admin' },
      payload: {
        userId: user1.id
    ...
    // Check occupiedBeds after removal
    const dormitoryAfterRemoval = await system.storage.findOne('Dormitory',
      MatchExp.atom({ key: 'id', value: ['=', dormitoryId] }),
      undefined,
      ['id', 'name', 'occupiedBeds']
    )

    expect(dormitoryAfterRemoval.occupiedBeds).toBe(1)

    // Verify BedAssignmentRelations exist for remaining user
    const { BedAssignmentRelation } = await import('../backend')
    const bedAssignments = await system.storage.find(BedAssignmentRelation.name,
      MatchExp.atom({ key: 'target.id', value: ['=', dormitoryId] }),
      undefined,
    ...
    expect(bedAssignments.length).toBe(1)
    expect(bedAssignments[0].source.username).toBe('occupancyuser2')
    expect(bedAssignments[0].bedNumber).toBe(2)
    expect(bedAssignments[0].target.id).toBe(dormitoryId)
  })

  test('Dormitory.availableBeds computed property', async () => {
    /**
     * Test Plan for: Dormitory.availableBeds
     * Dependencies: Dormitory entity, bedCount property, occupiedBeds property 
     * Steps: 1) Create dormitory 2) Verify availableBeds = bedCount - occupiedBeds 3) Assign users to beds 4) Verify availableBeds decreases accordingly
     * Business Logic: bedCount - occupiedBeds
     */
    
    // Create a dormitory with 6 beds
    const dormitoryResult = await controller.callInteraction('CreateDormitory', {
      user: { id: 'admin' },
      payload: {
        name: 'Available Beds Test Dormitory',
        bedCount: 6,
        building: 'V',
        floor: 2
      }
    ...
    const dormitory = await system.storage.findOne('Dormitory',
      MatchExp.atom({ key: 'name', value: ['=', 'Available Beds Test Dormitory'] }),
      undefined,
      ['id', 'name', 'bedCount', 'occupiedBeds', 'availableBeds']
    )

    // Initially: bedCount=6, occupiedBeds=0, availableBeds=6
    expect(dormitory.bedCount).toBe(6)
    ...
    const user1Result = await controller.callInteraction('CreateUser', {
      user: { id: 'admin' },
      payload: {
        username: 'availableuser1',
        email: 'available1@example.com',
        password: 'password123',
        fullName: 'Available User 1',
        role: 'student'
      }
    })
    ...
    const user1 = await system.storage.findOne('User',
      MatchExp.atom({ key: 'username', value: ['=', 'availableuser1'] }),
      undefined,
      ['id', 'username']
    )

    // Assign user 1 to bed
    const assign1Result = await controller.callInteraction('AssignUserToBed', {
      user: { id: 'admin' },
      payload: {
        userId: user1.id,
    ...
    // Check after first assignment: bedCount=6, occupiedBeds=1, availableBeds=5  
    const dormitoryAfter1 = await system.storage.findOne('Dormitory',
      MatchExp.atom({ key: 'id', value: ['=', dormitory.id] }),
      undefined,
      ['id', 'bedCount', 'occupiedBeds', 'availableBeds']
    )

    expect(dormitoryAfter1.bedCount).toBe(6)
    expect(dormitoryAfter1.occupiedBeds).toBe(1)
    expect(dormitoryAfter1.availableBeds).toBe(5)

    // Create and assign two more users
    const user2Result = await controller.callInteraction('CreateUser', {
      user: { id: 'admin' },
      payload: {
        username: 'availableuser2',
        email: 'available2@example.com',
        password: 'password123',
        fullName: 'Available User 2',
        role: 'student'
      }
    })
    ...
    const user3Result = await controller.callInteraction('CreateUser', {
      user: { id: 'admin' },
      payload: {
        username: 'availableuser3',
        email: 'available3@example.com',
        password: 'password123',
        fullName: 'Available User 3',
        role: 'student'
      }
    })
    ...
    const user2 = await system.storage.findOne('User',
      MatchExp.atom({ key: 'username', value: ['=', 'availableuser2'] }),
      undefined,
      ['id', 'username']
    )

    const user3 = await system.storage.findOne('User',
      MatchExp.atom({ key: 'username', value: ['=', 'availableuser3'] }),
      undefined,
      ['id', 'username']
    )

    // Assign users 2 and 3 to beds
    ...
    // Check after all assignments: bedCount=6, occupiedBeds=3, availableBeds=3
    const dormitoryFinal = await system.storage.findOne('Dormitory',
      MatchExp.atom({ key: 'id', value: ['=', dormitory.id] }),
      undefined,
      ['id', 'bedCount', 'occupiedBeds', 'availableBeds']
    )

    expect(dormitoryFinal.bedCount).toBe(6)
    expect(dormitoryFinal.occupiedBeds).toBe(3)
    expect(dormitoryFinal.availableBeds).toBe(3)

    // Remove one user and verify available beds increase
    ...
    // Check after removal: bedCount=6, occupiedBeds=2, availableBeds=4
    const dormitoryAfterRemoval = await system.storage.findOne('Dormitory',
      MatchExp.atom({ key: 'id', value: ['=', dormitory.id] }),
      undefined,
      ['id', 'bedCount', 'occupiedBeds', 'availableBeds']
    ...
    expect(dormitoryAfterRemoval.bedCount).toBe(6)
    expect(dormitoryAfterRemoval.occupiedBeds).toBe(2)
    expect(dormitoryAfterRemoval.availableBeds).toBe(4)
  })
}) 