...
import { RecordScoreForm, User, DormitoryMember, ScoreRecord } from '../types';
...
export function ScoreManagement({}, { createElement }: RenderContext) {
  ...
  const loadData = async () => {
    try {
      ...
      if (user) {
        ...
        const [membersData, membershipData, recordsData] = await Promise.all([
      ...
      console.error('Failed to load score management data:', err);
  ...
  const getCurrentUserDormitoryMembers = () => {
    ...
    if (user?.role === 'student' && membership?.role === 'leader') {
      return allMembers.filter(m => 
        m.dormitory.id === membership.dormitory.id && m.status === 'active'
      );
    }
    return [];
  };
  ...
  const getDormitoryScoreRecords = () => {
    const dormitoryMembers = getCurrentUserDormitoryMembers();
    const memberIds = dormitoryMembers.map(m => m.id);
    const allRecords = scoreRecords();
  ...
  const getStatistics = () => {
    ...
    return {
      totalMembers: dormitoryMembers.length,
      totalRecords: records.length,
      averageScore: dormitoryMembers.length > 0 ? 
        Math.round(dormitoryMembers.reduce((sum, m) => sum + m.score, 0) / dormitoryMembers.length) : 0,
      atRiskMembers: dormitoryMembers.filter(m => m.score < -50).length,
  ...
  const handleRecordScore = async () => {
    try {
      ...
      if (!form.memberId || !form.reason.trim() || form.points === 0) {
        alert('请填写完整的积分记录信息');
        return;
      }

      await interactionSDK.recordScore(form.memberId, form.points, form.reason, form.category);
      
      showRecordModal(false);
      resetForm();
      
      // Reload data to show new score record
      await loadData();
  ...
  const renderContent = () => {
    ...
    const isDormitoryLeader = user.role === 'student' && membership?.role === 'leader';

    if (!isDormitoryLeader && user.role !== 'admin') {
      return (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', padding: '20px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              <Select
                ...
                options={dormitoryMembers.map(member => ({
            ...
            <div style={{ 
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                ...
                <Input
                  ...
                  onChange={(value) => scoreForm({ ...scoreForm(), points: Number(value) })}
                />
              </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              <Input
                ...
                onChange={(value) => scoreForm({ ...scoreForm(), reason: value })}
              />
            </div>
            ...
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
              ...
              <Button 
                ...
                style={{ backgroundColor: '#1890ff', color: 'white', border: 'none' }}
                disabled={submitting() || !scoreForm().memberId || !scoreForm().reason.trim() || scoreForm().points === 0}
              >
                {submitting() ? '记录中...' : '记录积分'}
              </Button>
            </div>
