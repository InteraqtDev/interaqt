...
export class UpdateExecutor {
    ...
    async updateSameRowData(entityName: string, matchedEntity: Record, newEntityDataWithDep: NewRecordData, events?: RecordMutationEvent[]) {
        ...
        const columnAndValue = allSameRowData.map(({field, name, value, fieldType, valueType, args}: { field: string, name?: string, value: string, fieldType?: string, valueType?: string, args?: object }) => (
            {
                field,
                ...
                value: value,
                // fieldType/valueType/args 让 buildUpdateSQL 走 prepareFieldValue（json 规范序列化、
                // timestamp 方言形态、扩展 codec args），与 create 路径一致。
                fieldType,
                valueType,
                args,
            }
