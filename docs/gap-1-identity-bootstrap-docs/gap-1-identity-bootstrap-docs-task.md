<!-- generated-by: cyclic-task-prompt | template-sha256: 43a33f632926 | generated-at: 2026-08-30 -->
# interaqt — 项目画像
...
- 设计文档：`docs/gap-1-identity-bootstrap-docs/gap-1-identity-bootstrap-docs.md`
- 设计评审：`docs/gap-1-identity-bootstrap-docs/gap-1-identity-bootstrap-docs-review.md`
- 实现审计：`docs/gap-1-identity-bootstrap-docs/gap-1-identity-bootstrap-docs-audit.md`
- 运行记录：`docs/gap-1-identity-bootstrap-docs/retro.md`
...
## Task 1 GAP-1 身份自举与系统事件源的文档缺口修复

### 背景

`agentspace/output/paradigm-gaps-review-2026-08-29.md` §2 对 GAP-1（身份自举与按自然键的幂等创建）的评审结论是：能力面已由 4.10.0 基本关闭，剩余的是**文档缺口**——使用指南把 `Interaction` 当作事件的唯一正统入口，无主体入口（注册、验证码签发、合作方开通）的作者找不到正规写法，只好退回手写存储原语。修复方式是文档与示例，不是新原语。评审基准为主干 `94a9194`（版本 4.10.0）。

生成本任务时已在当前工作树（`0249556`）复核以下事实；设计阶段应对当前 HEAD 再次核对，不得只复述本节：

1. **幂等创建能力已存在。** `Entity.identity` 提供按自然键的 set-semantic 创建：冲突方在同一事务内按键查回既有行并返回观察结果，不报错、不发 create 事件（写路径汇合在 `src/storage/erstorage/CreationExecutor.ts` 的 `createIdentityRecord`）。「重复即类型化失败」分支由 `UniqueConstraint` 提供，两者在同一属性集上互斥且声明期报错（`src/core/Entity.ts` 的 `normalizeEntityIdentity`）。
2. **匿名 / 系统事件源能力已存在。** 核心层 `EventSource.create` 的参数类型完全由调用方决定、不含 user 概念（`src/core/EventSource.ts:80` 的注释即把无 user 的事件源称为 anonymous EventSources）；`Controller.dispatch` 入口不校验 user。内建 `Interaction` 把 `user` 声明为必填只是其自身的类型契约。
3. **能力残余**（评审要求显式成文，当前主要靠读源码得知）：`Relation` 的创建面没有 `identity` 参数（`src/core/Relation.ts` 全文无 identity）；filtered / merged 实体声明 identity 在声明期报错（`src/core/Entity.ts:301-310`）；MySQL 在 setup 期 fail-fast（`src/storage/erstorage/Setup.ts:746-754` 的 `assertMysqlDoesNotSupportIdentity`）。
4. **文档缺口已核实。** `agent/agentspace/knowledge/usage/` 全部指南（00–20）中没有任何 `EventSource.create`、anonymous / 匿名事件源内容；`01-core-concepts.md:107` 把 Interaction 描述为数据变化的 "the **only source** of data changes"。

### 要求

1. **系统 / 匿名事件源正式配方。** usage 知识库必须包含一条正式配方：自建事件实体 + 核心 `EventSource.create` + 自定义 `admit`，含至少一个无主体场景（注册 / 验证码签发 / 合作方开通同类）的完整可运行示例，并说明与 `Interaction` 的分工（后者的契约包含 user 与 payload 校验）。配方中的每个代码形态必须被可运行的测试钉住：优先引用既有测试（如 `tests/runtime/eventSource.spec.ts`），形态未被覆盖时新增测试。
2. **无框架级 user 闸门的成文化。** 在讲授 Interaction 与权限的指南位置明确：`Controller.dispatch` 入口不校验 user；对 `Interaction` 而言，拦住匿名调用的只有用户自己声明的 Condition。表述不得让读者把 user 校验误当作框架保证。
3. **能力残余的显式说明。** 在 identity 相关文档位置明确背景第 3 条列出的三项限制。设计期先盘点现有覆盖（usage 指南、generator 指南、`AGENTS.md`），只补缺、不重复；同一规则只在一个权威位置完整定义，其余位置引用。
4. **「唯一入口」叙述的一致性。** 修正与要求 1 配方相矛盾的表述（起点是 `01-core-concepts.md:107`）：准确的说法是数据变化的唯一入口是经 `Controller.dispatch` 触发的 EventSource，`Interaction` 是内建的、面向用户交互的 EventSource。完成后知识库不得留下自相矛盾的入口叙述；同类表述必须按同一根因一次查全（不限于已举出的那一处）。
5. **观察路径判别配方。** 「实插 vs 观察」的判别两步法（`DispatchResponse.effects` 中有无 create 事件 + 提交后按键查询）必须在 usage 知识库中有正式配方；已有覆盖时补齐交叉引用即可。
6. **范围边界（硬性约束）。** 本任务不新增、不修改任何框架 API 与运行时行为（`src/` 不改动；新增测试与文档不受此限）。评审对 GAP-1 第 3 条建议中的 `DispatchResponse` 解析结果通道明确不做——评审已裁定无实测痛点前不建。设计调研若发现评审的事实陈述与当前主干不符，在设计文档中记录更正并按实际现状执行，不扩大实现范围。

请先完成设计，不要实施生产代码。任务特定说明：

1. 输入文档：`agentspace/output/paradigm-gaps-review-2026-08-29.md` 的 §2 GAP-1 一节与 §4 第 1、2 条更正。来源清单 `prompt/output/interaqt-paradigm-gaps-2026-08-29.md` 只作背景理解；事实以评审文档与当前源码为准，两者冲突时以源码为准并在设计文档中记录。
2. 背景中的行号在评审基准（`94a9194`）与生成时工作树（`0249556`）上一致；设计期引用前应复核行号漂移，定位以符号名为准（`normalizeEntityIdentity`、`assertMysqlDoesNotSupportIdentity`、`createIdentityRecord`）。
3. 设计期待验证事项：(a) usage / generator 指南与 `AGENTS.md` 对三项能力残余、观察判别两步法的现有覆盖清单——要求 3 / 要求 5 的补缺范围以盘点结果为准；(b) 既有测试对要求 1 配方各代码形态的覆盖程度（`tests/runtime/eventSource.spec.ts`、`tests/runtime/applicationIdentity.spec.ts` 等）；(c) `Controller.dispatch` 入口校验清单（评审称只有四项且不含 user，须核对当前源码）。
4. usage 指南（`agent/agentspace/knowledge/usage/`，英文正文）的读者是下游应用工程师与生成应用的 agent 工作流：配方必须自包含、可直接执行，遵循指南既有的结构、编号与语言。所有书面产出遵守 `AGENTS.md` § "Plain professional language"。
5. 本任务以文档与示例测试为主，里程碑数量宜小（建议 2–3 个）。新增测试按 `AGENTS.md` § Testing 编写（通过 dispatch 测试、优先 PGLiteDB、`storage.find`/`findOne` 显式 attributeQuery）；不触及 driver / storage 写路径与迁移，无需真实 PostgreSQL 套件。
6. 文档改动完成后，检查 `agent/agentspace/knowledge/usage/README.md` 与相关指南间的交叉引用是否需要同步。
...
7. 不要执行下面任何 additional task。使用 `Please execute docs/gap-1-identity-bootstrap-docs/gap-1-identity-bootstrap-docs-task.md Task 1 additional task 1.` 作为 prompt 启动一个新的 chat，只要启动你自己就可以结束了，不需要等待。**使用 `prompt/skill/new-waku-session.md` 中的方法来启动新的 chat，启动参数：`--harness claude --model GLM-5.3`。**
...
5. 覆盖写入 `docs/gap-1-identity-bootstrap-docs/gap-1-identity-bootstrap-docs-review.md`，结论只能是：
   - `通过`
   - `需要修订`
   - `目标不可实现`
6. 不修改设计文档。
7. 使用 `Please execute docs/gap-1-identity-bootstrap-docs/gap-1-identity-bootstrap-docs-task.md Task 1 additional task 2.` 作为 prompt 启动一个新的 chat，只要启动你自己就可以结束了，不需要等待。**使用 `prompt/skill/new-waku-session.md` 中的方法来启动新的 chat，启动参数：`--harness claude --model GLM-5.3`。**

### additional task 2

你是设计裁决者。深度理解 Task 1 和原设计 `docs/gap-1-identity-bootstrap-docs/gap-1-identity-bootstrap-docs.md`，然后逐条核验 `docs/gap-1-identity-bootstrap-docs/gap-1-identity-bootstrap-docs-review.md` 中的问题，不得直接接受评审结论。遵守「设计复审条件」及以下流程：
...
7. 按顺序评估并执行（只走命中的第一条分支）：
  ...
  - 若本轮没有采纳任何「需要复审的问题」：设计通过。设置 `status: 实现中`，以初始里程碑数计算 `N = 5 × M`，然后使用 `Please execute docs/gap-1-identity-bootstrap-docs/gap-1-identity-bootstrap-docs-task.md Task 1 additional task 3.` 作为 prompt 启动一个新的 chat，只要启动你自己就可以结束了，不需要等待。**使用 `prompt/skill/new-waku-session.md` 中的方法来启动新的 chat，启动参数：`--harness claude --model GLM-5.3`。**
  - 若有采纳问题且 `d < 15`：使用 `Please execute docs/gap-1-identity-bootstrap-docs/gap-1-identity-bootstrap-docs-task.md Task 1 additional task 1.` 作为 prompt 启动一个新的 chat，只要启动你自己就可以结束了，不需要等待。**使用 `prompt/skill/new-waku-session.md` 中的方法来启动新的 chat，启动参数：`--harness claude --model GLM-5.3`。**
  - 若 `d = 15`：完成一次全局修订和六类条件自检。不存在已知阻塞问题时按上面的设计通过分支进入实现；仍存在已知阻塞问题时设置 `status: 协议失败`，记录原因。终止，不启动新的 chat。不得自行提高预算或要求用户选择方案。

### additional task 3

你是实现者。深度理解并实施 `docs/gap-1-identity-bootstrap-docs/gap-1-identity-bootstrap-docs.md`。你是实现循环中的**实现轮**，与 additional task 4 的**审计轮**交替执行。严格遵守以下流程：
...
8. 不得把里程碑标记为 `已完成`。
9. 本轮结束时**一律**使用 `Please execute docs/gap-1-identity-bootstrap-docs/gap-1-identity-bootstrap-docs-task.md Task 1 additional task 4.` 作为 prompt 启动新的 chat（无论你认为完成与否，都交给审计轮判定，不再自行连续启动 additional task 3）。只要启动就可以结束，不需要等待。**使用 `prompt/skill/new-waku-session.md` 中的方法来启动新的 chat，启动参数：`--harness claude --model GLM-5.3`。**

### additional task 4

你是独立实现审计者。深度理解 `docs/gap-1-identity-bootstrap-docs/gap-1-identity-bootstrap-docs.md` 并对实现进行对抗性 review。你是实现循环中的**审计轮**，与 additional task 3 的**实现轮**交替执行。不得信任里程碑状态或实现者的结论。按以下顺序执行：
...
9. 覆盖写入 `docs/gap-1-identity-bootstrap-docs/gap-1-identity-bootstrap-docs-audit.md`。
10. 按审计结论更新状态：
...
11. 按顺序评估并执行（只走命中的第一条分支）：
  ...
  - 未完成且 `k < N`：使用 `Please execute docs/gap-1-identity-bootstrap-docs/gap-1-identity-bootstrap-docs-task.md Task 1 additional task 3.` 作为 prompt 启动一个新的 chat，只要启动你自己就可以结束了，不需要等待。**使用 `prompt/skill/new-waku-session.md` 中的方法来启动新的 chat，启动参数：`--harness claude --model GLM-5.3`。**
