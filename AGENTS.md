...
- **Generative suites (r29, expanded r33)** — expand the relevant pool when touching the corresponding subsystem:
 - `tests/storage/matchSemanticsFuzz.spec.ts` — random boolean match trees (value atoms with NULL three-valued logic × exist atoms incl. nesting × AND/OR/NOT) reconciled against an independent in-memory evaluator (`FUZZ_MATCH_SEED_START/COUNT`, `FUZZ_MATCH_EXPRS`); run when touching match compilation (MatchExp/SQLBuilder/QueryExecutor). This is the query-semantics oracle: driver-differential suites run the same compiled SQL on both sides and are structurally blind to compiler-level semantic bugs (r35 retrospective)
...
5. **For fatal bugs that escaped existing tests, record the escape analysis.** Write *why the test system missed it* (retrospective in `agentspace/output/`), and turn the lesson into a mechanism — a new axis, an oracle, an invariant — not prose. See `agentspace/output/r17-test-blindness-retrospective.md` and `r18-test-blindness-retrospective.md` for the method and precedents. Two oracle rules from r35: (a) a reconciliation between two outputs of one shared implementation proves consistency, not correctness — every relative-oracle group needs at least one absolute assertion grounded in independently-derived facts; (b) a recorded "known broken / risk" entry must carry an executable pin (a failing assertion or `test.fails`) whose assertion encodes the hypothesized mechanism — prose registrations with unverified mechanism hypotheses misdirected diagnosis for ten rounds (r25#7).
