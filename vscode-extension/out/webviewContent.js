...
function getWebviewContent(data, _webview) {
...
function generateLifecycleSection(lifecycle) {
    ...
    const creationInteractions = creation?.creationInteractions?.map(i => `<div class="interaction-item"><span class="interaction-name">${i.name}</span></div>`).join('') || '';
    return `
...
function generateVerificationSection(verification) {
