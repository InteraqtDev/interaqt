...
interface InteractionsDesign {
    ...
    interaction_matrix?: {
        ...
        by_data_entity?: Record<string, string[]>;
    };
    coverage_analysis?: {
...
interface Interaction {
    ...
    specification: {
        ...
        data: {
            reads?: string[];
            creates?: DataOperation[];
            updates?: DataOperation[];
            deletes?: DataOperation[];
        };
...
export function getInteractionsWebviewContent(data: InteractionsDesign, _webview: vscode.Webview): string {
    ...
    // Group by role
    const roleGroups: Record<string, Interaction[]> = {};
...
function generateInteractionCards(interactions: Interaction[]): string {
                ...
                <div class="payload-grid">
                    ${Object.entries(spec.payload).map(([name, field]) => `
                        <div class="payload-item">
                            ...
                            <span class="payload-type">${escapeHtml((field as PayloadField).type || 'any')}</span>
                            <span class="payload-desc">${escapeHtml((field as PayloadField).description || '')}</span>
                            <span class="payload-required ${(field as PayloadField).required ? 'yes' : 'no'}">${(field as PayloadField).required ? 'required' : 'optional'}</span>
                        </div>
                    `).join('')}
                </div>
            </div>
...
function generateDataOperationsHtml(data: { reads?: string[]; creates?: DataOperation[]; updates?: DataOperation[]; deletes?: DataOperation[] }): string {
...
function generateRoleGroups(roleGroups: Record<string, Interaction[]>): string {
...
function generateMatrixView(title: string, icon: string, matrix: Record<string, string[]>): string {
...
function generateCoverageView(coverage: InteractionsDesign['coverage_analysis'], percentage: number): string {
    const cov = coverage || {};
