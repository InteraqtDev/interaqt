...
1. match 语义 fuzz 生成域进一步扩张：between / like / 引用值 / filtered 视图作为查询根 /
   对称段——独立轮，扩张时按套件自身种子宇宙重验。
2. 迁移两个 fuzz 的 schema 域加入 filtered 实体链（Transform → filtered → 聚合），
   直接机器化 F-4 的形态家族。
3. atomic API 的「方法 × 值类型」全矩阵枚举审计（S-1 类的机器化；本轮已点探 CAS×boolean）。
