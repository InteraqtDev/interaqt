...
> - 追加(不做向后兼容决策的延伸):移除了 legacy 计算 id 语义归一化(`normalizePreviousComputationManifest`/`hasSameSemanticComputationShape`)。generator 版本门禁已使其原始场景不可达,残余可达路径是对"同 dataContext 换计算类型"的隐式同一性推断。替代为 fail-fast:迁移身份只接受显式声明的类型名(`computationType.displayName`/`args._type`/args 类或 handle 类的 `displayName`),回退到可被压缩器改写的类名时直接报错;类型名变化按 removed + added 走正常审阅重建。
...
- ~~legacy 计算 id 归一化(`normalizePreviousComputationManifest`)对压缩产物导致的类型名漂移做了语义匹配~~(修复轮次中按"不做向后兼容"决策移除:它的目标场景已被 generator 版本门禁拦截,残余路径是隐式身份推断。替代方案是 fail-fast——计算类型必须显式声明 `displayName` 或 `_type`,类名不再参与迁移身份)。
