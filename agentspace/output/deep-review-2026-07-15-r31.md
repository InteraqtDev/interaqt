...
| 硬化（静默死转移/过触发家族，A 的枚举顺产） | H7 | trigger/eventDep 模式外层字段无守卫：typo 字段 trigger 轨永不触发、eventDep 轨静默丢弃过滤条件（含不支持的 keys） | 已修复 |
...
| combined x:1 的 ORDER BY 排序键不设 link-id 门（match 面 r28 已设 `&`.id IS NOT NULL 守卫，Modifier 面没有）——孤儿同住行按幻影列值排序而非 NULL | 未定谳（既未验证也未证伪）：损坏前提是孤儿同住行（r28 后合法产生路径收窄为 hub 亡故等窄面），复现需要 r28 种子级装配；预言机缺口面（fuzzer 不断言 orderBy）已知。列为下轮 orderBy 预言机（对账排序结果与逻辑快照排序）候选 |
| filtered 属性路径在 orderBy / isReferenceValue 中不做 base 拓扑 rebase（attributeQuery 面 r30 已 rebase）→ JOIN 到不存在的 filtered link 表 | fail-loud（no such column 级报错），非静默；与「嵌套 x:1 谓词下推」同属查询编译面缺口，合并处理 |
| 迁移期真实写入的 mutation events 仍会派发给残留的 `storage.listen` 回调（MigrationScheduler 用返回值事件流，两条轨不一致） | 文档化边界（迁移期不应有业务监听者在场）；enforcement 需要迁移引擎的监听隔离，与迁移专项同押 |
