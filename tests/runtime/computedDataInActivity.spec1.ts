...
import {Controller, ActivityCall, ActivityGroupNode, MonoSystem} from "src/runtime/index.js";
import {
    Activity,
    createInstances,
    Entity,
...
describe('computed data in activity', () => {
    ...
    beforeEach(async () => {
        ...
        const userARef = await system.entities.create('User', {name: 'A', age:11})
        userAId = userARef.id

        const userBRef = await system.entities.create('User', {name: 'B', age:12})
        userBId = userBRef.id

        const userCRef = await system.entities.create('User', {name: 'C', age:13})
        userCId = userCRef.id

        const userDRef = await system.entities.create('User', {name: 'D', age:14})
        userDId = userDRef.id
    })
    ...
    test('make friend activity', async () => {
        // 0. 验证初始数据
        const userA: User = {
            ...await system.entities.findOne('User', MatchExp.atom({
                key:'id',
                value: ['=', userAId]
            }), undefined, ['*']),
            roles:['user']
        ...
        const totalFriendRelation = await system.entities.get('state','totalFriendRelation')
        const everyRequestHandled = await system.entities.get('state','everyRequestHandled')
        const anyRequestHandled = await system.entities.get('state','anyRequestHandled')
        ...
        const userB1 = (await system.entities.findOne('User', MatchExp.atom({
                key:'id',
                value: ['=', userBId]
            }), undefined, ['*']))
        expect(userB1.totalUnhandledRequest).toBe(1)

        const userA1 = (await system.entities.findOne('User', MatchExp.atom({
            key:'id',
            value: ['=', userAId]
        }), undefined, ['*']))
        expect(userA1.everySendRequestHandled).toBeFalsy()
        expect(userA1.anySendRequestHandled).toBeFalsy()
        const everyRequestHandled1 = await system.entities.get('state','everyRequestHandled')
        ...
        const userB2 = (await system.entities.findOne('User', MatchExp.atom({
            key:'id',
            value: ['=', userBId]
        }), undefined, ['*']))
        ...
        const userA2 = (await system.entities.findOne('User', MatchExp.atom({
            key:'id',
            value: ['=', userAId]
        }), undefined, ['*']))
        ...
        const everyRequestHandled2 = await system.entities.get('state','everyRequestHandled')
        expect(everyRequestHandled2).toBeTruthy()
        const anyRequestHandled2 = await system.entities.get('state','anyRequestHandled')
        expect(anyRequestHandled2).toBeTruthy()

        const totalFriendRelation1 = await system.entities.get('state','totalFriendRelation')
        ...
        const userB3 = (await system.entities.findOne('User', MatchExp.atom({
            key:'id',
            value: ['=', userBId]
        }), undefined, ['*']))
        expect(userB3.totalFriendCount).toBe(0)

        const userA3 = (await system.entities.findOne('User', MatchExp.atom({
            key:'id',
            value: ['=', userAId]
        }), undefined, ['*']))
        ...
        const totalFriendRelation2 = await system.entities.get('state','totalFriendRelation')
        ...
        const userB12 = (await system.entities.findOne('User', MatchExp.atom({
            key:'id',
            value: ['=', userBId]
        }), undefined, ['*']))
        expect(userB12.totalUnhandledRequest).toBe(2)

        const everyRequestHandled12 = await system.entities.get('state','everyRequestHandled')
        expect(everyRequestHandled12).toBeFalsy()
        const anyRequestHandled12 = await system.entities.get('state','anyRequestHandled')
        ...
        const userB14 = (await system.entities.findOne('User', MatchExp.atom({
            key:'id',
            value: ['=', userBId]
        }), undefined, ['*']))
        expect(userB14.totalUnhandledRequest).toBe(0)

        const everyRequestHandled14 = await system.entities.get('state','everyRequestHandled')
        expect(everyRequestHandled14).toBeTruthy()
        const anyRequestHandled14 = await system.entities.get('state','anyRequestHandled')
