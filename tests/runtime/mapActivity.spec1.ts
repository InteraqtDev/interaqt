...
describe('map activity', () => {
    ...
    beforeEach(async () => {
        ...
        const userARef = await system.entities.create('User', {name: 'A', age:10})
        userAId = userARef.id
    ...
    test('make friend activity', async () => {
        // 0. 验证初始数据
        const userA: User = {
            ...await system.entities.findOne('User', BoolExp.atom({
                key:'id',
                value: ['=', userAId]
            }), undefined, ['*']),
            roles:['user']
        ...
        const requests1 = await controller.system.entities.find('Request', requestMatch, undefined, ['handled', ['activity', {attributeQuery: ['id']}], ['from',{attributeQuery:["name"]}], ['to', {attributeQuery:["name"]}], ['message', {attributeQuery:["content"]}]])
        ...
        const requests2 = await controller.system.entities.find('Request', requestMatch, undefined, ['handled', ['activity', {attributeQuery: ['id']}], ['from',{attributeQuery:["name"]}], ['to', {attributeQuery:["name"]}], ['message', {attributeQuery:["content"]}]])
        expect(requests2.length).toBe(1)
        expect(!!requests2[0].handled).toBeFalsy()
        expect(requests2[0].activity.id).toBe(activityId)

        const userB1 = (await system.entities.findOne('User', BoolExp.atom({
                key:'id',
                value: ['=', userBId]
            }), undefined, ['*']))
        expect(userB1.totalUnhandledRequest).toBe(1)

        const userA1 = (await system.entities.findOne('User', BoolExp.atom({
            key:'id',
            value: ['=', userAId]
        }), undefined, ['*']))
        ...
        const requests3 = await controller.system.entities.find('Request', requestMatch, undefined, ['handled', ['activity', {attributeQuery: ['id']}], ['from',{attributeQuery:["name"]}], ['to', {attributeQuery:["name"]}], ['message', {attributeQuery:["content"]}]])
        expect(requests3.length).toBe(1)
        expect(!!requests3[0].handled).toBeTruthy()
        expect(requests3[0].activity.id).toBe(activityId)

        const userB2 = (await system.entities.findOne('User', BoolExp.atom({
            key:'id',
            value: ['=', userBId]
        }), undefined, ['*']))
        ...
        const userA2 = (await system.entities.findOne('User', BoolExp.atom({
            key:'id',
            value: ['=', userAId]
        }), undefined, ['*']))
        ...
        const relationName = controller.system.entities.getRelationName('User', 'friends')
        const friendRelations = await controller.system.entities.findRelationByName(relationName, undefined, undefined, [['source', {attributeQuery: ['name', 'age']}], ['target', {attributeQuery: ['name', 'age']}]])
        ...
        const friendRelations2 = await controller.system.entities.findRelationByName(relationName, undefined, undefined, [['source', {attributeQuery: ['name', 'age']}], ['target', {attributeQuery: ['name', 'age']}]])
        expect(friendRelations2.length).toBe(0)

        const userB3 = (await system.entities.findOne('User', BoolExp.atom({
            key:'id',
            value: ['=', userBId]
        }), undefined, ['*']))
        expect(userB3.totalFriendCount).toBe(0)

        const userA3 = (await system.entities.findOne('User', BoolExp.atom({
            key:'id',
            value: ['=', userAId]
        }), undefined, ['*']))
