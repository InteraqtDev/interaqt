...
| 迁移破坏性删除的审计位置（r32 引入） | destructive-scope 的精确性只能在两个位置成立：**发现层 = 回滚事务内真实执行**（级联依赖的删除取决于上游迁移后状态，分析性一阶评估对链式依赖必然失真）、**执行层 = 删除以 delete 事件出账 + 提交前双向对账**。「入口预测式断言」对级联删除是结构性失明——批准侧永远无法先验给出正确 ids（r30-E 的 kill-resume 死循环本质）。scope 断言的测试矩阵：seed（一阶）/ 链式依赖 Transform / **级联依赖 `_isDeleted_` 宿主（分析性求值给空集的格）** / 模拟不可用的回退环路（错误报告 → 单次再批准收敛） | r30 E（unapprovable dead loop）；r32 B1–B4 |
