...
**新增轴的同时必须对既有同族格做一次回灌审计**（r35 复盘 §三：r32 立了「成员资格
进入+退出双向」轴，但既有 enter-only 测试未按新轴升级，退出面的 fatal 因此多活三轮——
立轴不回灌等于只保护未来的格、放过存量的格）。
...
| 计算轨道（机制轴） | 数据驱动（dataDeps）/ **事件驱动（StateMachine trigger / Transform eventDeps）** / **migration 签名（第三个读者：同一声明在 manifest 里是否可见）**。记录级声明：`Entity.retention` → `records[].retention`；`Entity.identity` → `records[].applicationIdentity`（不得写入 `records[].identity`，该键是 MigrationIdentity） | r18 F-1、r18 F-2 |
...
| 异步产出轨交错 × freshnessKey（r30 引入，r33 机器化，r34 语义分裂收口） | async 计算的「最后产出胜出」不变量必须对**同一数据上下文上全部产出轨的任意交错**成立：产出轨 = async task 完成轨 / **同步 compute 直出轨** / **resolved（asyncReturn 短路）轨** / skip / **迁移重建轨（writeComputationResult/Patch 直写——r34-A5 起重建前按上下文作废旧纪元 task）**。不变量的**载体**决定写者枚举的可见范围：载在 task 行（代理）上时，绕过代理的同步/resolved/迁移写者对它天然不可见（r30-B：sync 新值后旧 pending task 完成仍被判"最新"并覆写）。**freshnessKey 承担两个语义必须显式区分（r34-A）**：排序分区（isLatestAsyncTask 按分区内最大 id，可被 args.freshnessKey 自定义）≠ 作废范围（invalidateUnappliedAsyncTasks 按**数据上下文身份**：property=宿主记录经 record link、global/entity/relation=独占整表）——按分区作废时自定义分区里的旧 task 存活并还魂 r30-B。**task↔record 1:1 link 的置换语义是第三个身份面（r34 fuzz 扩域顺产定谳）**：同宿主新 task 建链会抢走旧 task 的 link（宿主侧排他 replace），脱链行与宿主已删行同形态（record=null），投递时统一由 orphaned-record 守卫停放（skipped）——record 作废只删得到持链行，孤儿守卫兜底脱链行，两个机制合起来 = 无陈旧 apply、无毒丸（r34-B：此前裸 TypeError 永不收敛）。**r33 起本轴由 `asyncComputationGenerativeFuzz.spec.ts` 机器铺格，r34 扩域**：返回类型序列数据驱动、自定义 freshnessKey（偶数种子）、宿主删除 × 悬挂/脱链 task、daemon 对全状态 task 的投递语义对账（applied/stale/orphaned/already-handled/task-not-success 逐字）；值收敛 + task 行三面（status/partition/link）逐位对账。仍未进生成域：entity/relation 级 async、并发 daemon 投递 | r30 B（isLatestAsyncTask 只见 task 行；r2 TOCTOU 加固只枚举了"经过代理的写者"）；r34 A/A5/B（r30 诚实边界的定向探针出货：自定义分区还魂 r30-B、迁移纪元不作废、孤儿 task 毒丸——**登记的诚实边界是下一个 bug 的地址**） |
...
| 迁移破坏性删除的审计位置（r32 引入，r33 机器化） | destructive-scope 的精确性只能在两个位置成立：**发现层 = 回滚事务内真实执行**（级联依赖的删除取决于上游迁移后状态，分析性一阶评估对链式依赖必然失真）、**执行层 = 删除以 delete 事件出账 + 提交前双向对账**。「入口预测式断言」对级联删除是结构性失明——批准侧永远无法先验给出正确 ids（r30-E 的 kill-resume 死循环本质）。scope 断言的测试矩阵：seed（一阶）/ 链式依赖 Transform / **级联依赖 `_isDeleted_` 宿主（分析性求值给空集的格）** / 模拟不可用的回退环路（错误报告 → 单次再批准收敛）。**r33 起破坏性变异轨由 `migrationDestructiveFuzz.spec.ts` 机器铺格，r34 扩两格**：随机存量数据 × {Transform 收缩 / `_isDeleted_` 硬删除（link 级联 + 下游 Count）/ 空 fact 退役 / Count changed vs unchanged 决策 / 被拒绝的非空删除与类型变更 / **fact→computed takeover（discard-and-rebuild）** / **计算类型变更 Count→Summation（manifest id removed+added）**} × kill-resume；预言机 = 无关面逐字段保真 + 变异特定朴素重算 + 迁移后增量接线冒烟 | r30 E（unapprovable dead loop）；r32 B1–B4 |
| 共享关联记录 × 增量反查唯一性（r35 引入，A1 收口顺产） | 增量计算按事件反查数据行时，**查询条件必须完整表达"这次增量属于谁"**：关联实体可被多个宿主共享（n:n 共享 target / n:1 多宿主 / 深路径共享深层记录），无宿主约束的 `findOne(relation, {side.id})` 是「任取一条指向该实体的 link」——每个宿主的贡献状态读写同一条 link（聚合值静默算错）。**无唯一性保证的 findOne 语义上是"任取"，凡当"恰好那条"用皆潜伏缺陷**。掩蔽机理是**行扫描序**：PGLite/PG 的 MVCC 更新把行移到扫描序末尾，逐宿主处理恰好"轮转"到各自正确的 link；SQLite 稳定 rowid 序下确定性错——**扫描序敏感性是驱动差异轴的新取值**，计算层 fuzz 增设 `FUZZ_COMP_DRIVER=sqlite` 变体（nightly），值预言机只有在两种行序模型上都跑过才算覆盖该轴 | r35（aggregationTemplate update 分支缺宿主约束——PGLite-only 的计算 fuzz + 一致性矩阵全体失明；`aggregationSharedTargetUpdate.spec.ts` 固化，双驱动矩阵） |
| 执行路径断言（r35 引入，performance-debt-plan Phase 0） | 计算的**值正确**与**执行路径正确**是两个观察面：全量重算兜底会把路由/守卫缺陷折叠成"值对但慢"（filtered targetPath 家族在 r19–r32 十轮记录期间不可红-绿验证的根因）。`tests/runtime/helpers/perfProbe.ts` 提供确定性计数判据：**compute() 调用数 =0（增量声明形态）/ fullRecompute 信封 reason 归因 / SERIALIZABLE attempt 数 =0（无隔离级升级重启）**。凡声称"该形态走增量"的修复/回归，必须附计数断言（`filteredRelationAggregationIncrementality.spec.ts` 样板：成员资格 × 操作 × 聚合形态矩阵 + 对照组 + 已知恒全量形态的探针灵敏度自检）；性能敏感路径的改动应带该轴 | r35（A1 判据落地时发现记录已陈旧：风暴形态已被历轮修复消除，但从未有判据能证明——「值断言绿」≠「路径断言绿」） |
| 回调契约的同步性 × 声明形态（r35b 引入） | 每个 function 声明面的**消费方式（await 与否）是契约的一部分**：同步消费面（聚合 callback / `Property.computed` / `Property.defaultValue` / `Dictionary.defaultValue` / `RealTime.nextRecomputeTime` / `Custom.createState`/`planIncremental`）收到 async 函数时，Promise 被 `!!`（恒 true）/ `Number()`（NaN→0）/ JSON 序列化（"{}"）静默强转落库——类型系统（`Function`）与直觉（到处都是 async 回调）都不拦。收口：`PublicFieldDef.synchronous` 元数据 + `assertSynchronousFunctionArg`（声明期，@core klassValidation）+ 聚合消费点 thenable 守卫（`aggregationTemplate.assertSyncCallbackResult`，兜底 transpile 退化/同步函数返回 Promise）。**新增 function 声明面时必须显式决策 synchronous 标记**；异步合法面（Transform/Custom compute/StateNode.computeValue/RealTime.callback/Condition）在回归里正面钉住防过度收紧 | r35b（Count async callback 恒计满——`leaveRequestSimple.ts` 夹具自身就是活体样本（async computed 落 "{}"），存活 35 轮无人察觉；`reviewFixesR35.spec.ts` + taboo 格） |
| 隐式量化算子的否定语义（r35b 引入） | x:n 路径原子隐含存在量化（∃ 相关行满足谓词），**NOT 与量化子不对易**：¬(∃ 满足) ≠ ∃ 不满足。exist 原子的编译必须保证否定按根记录量化——父路径含 x:n 段时整条路径折叠进 EXISTS（`MatchExp.existAtomCorrelation` 收敛判定：'root' = 反向路径关联根 + 外层零 JOIN；'parent' = 全 x:1 / '&' / 对称段的既有编译），否则外层扇出行逐行求值 NOT EXISTS，「有一个不含匹配终端的中间行」就放行本应排除的根。**EXISTS 关联引用的解析作用域是独立缺陷格**：关联原子必须预解析绑定直接外层查询的别名（`isResolvedFieldReference`，含前缀链）——按 contextRootEntity（最外层根）路径解析在嵌套 EXIST 下绑错表（r25#7 真相）。矩阵：{正向, NOT} × 中段基数 {全 x:1, 1:n, n:n, 双 x:n} × 终段基数 {x:1, x:n} × **中段声明形态 {普通关系, filtered relation}** × {find, update 选择, delete 选择} × 双驱动（`existCorrelationScope.spec.ts`）。**登记边界**：对称段中间路径维持 'parent' 编译（NOT 语义仍按扇出行）；**中段 filtered relation 同样维持 'parent'**（rebased link 谓词 AND 在外层、与路径原子共享 JOIN 别名才有「同一条边」语义——root 折叠会把量化域扩大到全部 base 边，r35 收口自查的邻格探针当场抓获该回归，`hasRebasedPathPredicate` 内部标记收口）；x:n **值原子**（`groups.name = 'G1'`）的 NOT 仍是 SQL 三值 per-row 语义（无组根既不在正集也不在补集）——量化语义未显式声明的面，改动是契约决策不是修复 | r35b（NOT × 多段 exist 误报族 + 嵌套 exist 空集族——存活自 exist 引入；`existJoinPruning.spec.ts` 只铺了正向；filtered 中段 × exist 在既有全量测试中零覆盖） |
| 运行时派生定义 × 登记面 × 持有 Controller 图（controller-retention 任务引入） | 框架在 setup / migration / storage 编译期合成的派生定义（非用户声明）必须走**唯一非注册构造路径**（`Entity/Relation/Property.derive`，汇合点），不得进入 `X.instances` 用户声明注册表——注册表无移除路径，任何运行时 `X.create()` 合成点都是永久滞留格。取值 = 派生 Klass {Entity / Relation / Property} × 合成路径 {bound-state 属性注入（install 轨 / **migration 轨（`setup(false)` / `createMigrationBaseline` 各自再走一批）**）/ merged 编译（`__type` 判别列 / transformed item / 虚拟 base entity / **虚拟 base relation（仅 filtered-input-into-merged 触发——filtered-over-merged 不触发）** / rebase filtered input）} × 观察面 {注册表长度（经 `KlassByName` 快照）/ `stringifyAllInstances` 纯净性 / `WeakRef` 可回收性 / **闭包捕获物（bound-state `defaultValue` 不得捕获 `RecordBoundState`——它持有 controller；只捕获纯值）**}。**保留链有两族**：Property 直接链与 merged 派生 Entity/Relation 经 clone 图的间接链——只断言一族对另一族失明。**观察时点是判别维度**：断言必须在 setup 后、destroy 前捕获证据（destroy 后观察放过「setup 期登记 + destroy 期反注册」的截断式伪修复）。allowlist spec（`runtimeCreateSiteAllowlist.spec.ts`）把「运行时合成一律走 derive」机器化为源码结构谓词，新增运行时 `X.create()` 站点必须显式决定用户声明 vs 派生定义 | controller-retention（问题陈述只覆盖 Property 直接链；Entity/Relation 间接链由因果实验「只截断 Property 后三 WeakRef 仍存活」揭示——每轮 `Entity+3 / Relation+3+ / Property+9` 登记、Controller/System/Database 永久存活、PGLite 每轮 RSS +≈215 MB） |
| 迁移期状态图变更 × 作用域 × bound state 所有权 × rebuild 标志（migration-state-machine-state-remap 引入） | StateMachine 的持久化 `currentState` 是**受状态图约束的持久化事实**，不是「有默认值、可整体重置」的附属数据。状态图变更类型 {初始态改名（`stateSignature` 变、双真计划）/ **非初始态改名（`stateSignature` 不变——签名只看默认值，`rebuildState=false`，只看标志会漏）** / 删除节点（removed 无 added 配对）/ 仅新增节点（不需要映射）} × 作用域 {property 级 / global 级} × bound state 所有权 {输出路径不拥有（StateMachine 的 `currentState`、Custom `createState` 的应用语义状态——走映射/重置）/ **输出路径拥有（Transform 的 `sourceRecordId`/`transformIndex`、聚合值与逐项贡献、RealTime 时间戳——`ownsStateOnRebuild` 声明，重置即破坏：键坍缩→唯一索引崩溃或重复行）**} × rebuild 标志组合 {`rebuildState && rebuildOutput` / `rebuildState && !rebuildOutput`（state-only 项在 `if (!item.rebuildOutput) continue` 处跳出——扫描/映射必须挂在 continue 之前）/ `!rebuildState && rebuildOutput`（映射施加以「批准决策存在」为触发面，不能只看标志）}。**验收断言必须双面**：迁移后执行依赖该状态的 Interaction（`error` 为 undefined 且**输出值与 bound state 列都变化**）——只断言迁移计划/handler 返回值放过「迁移成功但下次交互静默 skip」的坏形态；**只改输出值不改状态列的错误实现必须被 currentState 列断言区分**。映射决策组合面（同 dataContext 的 computation 决策）{changed / state-only / **unchanged / unrebuildable（不播种计划→映射被静默丢弃，validateApprovedDiff 必须拒绝）**}。升级窗口（旧 manifest 无 `stateGraph` 字段→「不比较」降级）是独立格：映射要求不产生，合法性不变量（宽口径：rebuild 项触发即扫单列）是唯一防线。样板：`migrationStateMachineStateGraph.spec.ts`（31 用例）+ `postgresqlMigration.spec.ts` 的 `"PostgreSQL state graph mapping migration"` | migration-state-machine-state-remap 机制 A/B/C/D/G（迁移后下一次交互静默无效——`TransitionFinder.findNextState` 对未知名返回 null → `ComputationResult.skip()`，error undefined；PR #51 的两行修复只覆盖机制 A 且放大机制 D 暴露面） |
