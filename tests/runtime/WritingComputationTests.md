...
| 预言机读取面 × 名字形态（r30 引入，归因修正实验确立） | **覆盖 = 生成域 × 预言机读取面的乘积**：形状被生成过 ≠ 被观察过。按名字形态（base 名 / filtered entity/relation 名 / merged input 名）分身的概念，其**每个读取 API 面**都是独立的格：实体嵌套读取（经声明属性名）/ relation-name 读（findRelationByName）/ match 路径 / 计算 dataDep 读。r30-A 的形状（filtered relation over combined base）自 r29 起就在 fuzzer 生成域内被反复生成，8 条预言机全绿——因为没有一条**经 filtered 属性名做实体嵌套读取**（第 6 条只读 base 属性面、第 7 条只读 relation-name 面，而 relation-name 面恰好一直正确）。预言机/回归矩阵对每个读取面逐一铺点或显式豁免；「诚实边界」必须双侧登记（生成侧缺什么形状 + **观察侧缺什么面**）。**同一次修复内新增的多个判定点也是读者对**：必须验证它们读同一投影（r30-A：synthetic-`&` 判定读 filtered link、prune 判定读 base 名，同一 PR 内诞生并分叉）。**读取面 × 挂载深度（r31 扩展）**：同一读取面在不同结构深度有独立的挂载/读取实现点——filtered x:n 在**顶层**（findRecords 步骤 3，alias 正确）与在 **x:1 主干之下**（completeXToOneLeftoverRecords 补全枝干，r31 前按 base 名挂载）是两个格；「结果挂载 key」的约定（`alias \|\| attributeName`）必须在**全部**子查询结果挂载点收口（两处补全枝干 + 三处 link x:n + 顶层/goto/batched），r30 只修了读取父级 x:1 的面 | r30 A（fuzzer 第 7b 条落地：未修复代码 extended seed 2 一击变红，修复后 1–120 全绿）；r31 S1（同一 alias 约定的挂载侧缺口：filtered x:n under x:1 挂在 base 名下——filtered 名缺失 + 子集泄漏，7b 预言机只铺了顶层深度） |
| 异步产出轨交错 × freshnessKey（r30 引入） | async 计算的「最后产出胜出」不变量必须对**同一 freshnessKey 上全部产出轨的任意交错**成立：产出轨 = async task 完成轨 / **同步 compute 直出轨** / **resolved（asyncReturn 短路）轨** / skip / **迁移回填轨（writeComputationResult 直写）**。不变量的**载体**决定写者枚举的可见范围：载在 task 行（代理）上时，绕过代理的同步/resolved/迁移写者对它天然不可见（r30-B：sync 新值后旧 pending task 完成仍被判"最新"并覆写）。手写夹具的单线叙事恒只测单轨（asyncComputed.spec 的 async 与 resolved 分别在不同记录上）；交错序列（async→sync→旧 task 完成）是本轴的必测格。计算层生成式 fuzz 纳入 async 域前，该轴由 `review-fixes-2026-07-15-r30.spec.ts` 的交错回归 + 纯 async 对照守住 | r30 B（isLatestAsyncTask 只见 task 行；r2 TOCTOU 加固只枚举了"经过代理的写者"） |
