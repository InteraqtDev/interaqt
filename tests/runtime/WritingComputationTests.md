...
| 单事件 × 多监听扇出（r27 引入） | 同一计算声明 **N 个 dataDeps ⇒ N 个注册监听**；一次 mutation 命中多个监听时的执行次数必须显式决策：**property dep（自身或同一 targetPath）合并为一次**（两次调用收到同一事件对象，用户层无从去重——useLastValue 增量被双叠加、create 初始计算双跑）；**records dep 按 dep 独立执行**（不同 match 的 membership 判定 entered/left/skip 是各 dep 增量语义的一部分）。live Scheduler（buildComputationMutationListener）与 migration 链式 rebuild（queueEvents 入队去重 + runIncrementalRecompute 重放去重）是同一契约的两个消费方 | r27 F-2（双 property dep 单次 update 增量双跑；单次 create 初始计算双跑） |
...
| 行搬迁的携带深度（r27b 引入，fuzzer 首跑抓获） | flashOut/relocate 的物理行搬迁子树由 `getAttributeQueryDataForRecord` 的**递归深度**隐式决定：sameTableReliance 递归携带（随行搬运）、**notRelianceCombined 不下钻**（跨关系同住结构被清行销毁——link 消失零事件）。每个搬迁入口（host-attr steal / link-endpoint claim / relocate / create-ref 领养）× 被搬行的同住结构（reliance 依赖 / mergeLinks 同住 / 反向 merged FK）是独立的格；深度不足的组合必须 fail-fast（F-4/F-5 守卫），完整深度实现前不得放行。**reliance 生命周期契约（依赖只能随 owner 删除）的三条产生轨**：update 换绑（unlink 守卫，既有）/ addRelation·直建 link（F-4 守卫）/ create-ref 领养（F-4 守卫） | r27b F-4（置换静默销毁旧依赖）、F-5（跨关系同住 link 静默消失）；开放家族 G-1–G-5 见 quality-foundation-plan-r27.md §1.4 |
