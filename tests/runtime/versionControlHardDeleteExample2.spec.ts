  ...
  test('Style entity with version control using hard deletion', async () => {
    ...
    const versionUpdateState = StateNode.create({
      ...
      computeValue: (lastValue, mutationEvent) => {
        ...
        const info = event.value.raw;
        if (event.key === 'currentVersionInfo' && info.type === 'publish') {
          return info.version;
        }
        return lastValue;
      }
    });
    // publish 的时候，就全体使用最新的 version
    versionProperty.computation = StateMachine.create({
      states: [versionUpdateState],
      defaultState: versionUpdateState,
      transfers: [
        StateTransfer.create({ 
          current: versionUpdateState, 
          next: versionUpdateState, 
          trigger: { 
          ...
          computeTarget: async function(this: Controller, mutationEvent: RecordMutationEvent) {
            // 所有记录
            return this.system.storage.find('Style',
              undefined,
              undefined,
              ['*']
            )
    ...
    const hardDeletionProperty = Style.properties!.find(p => p.name === HARD_DELETION_PROPERTY_NAME)!;
        ...
        StateTransfer.create({ 
          ...
          computeTarget: async function(this: Controller, mutationEvent: RecordMutationEvent) {
            ...
            if (rollbackTo !== undefined) {
              const newVersion = mutationEvent.record!.value.raw.version;
              // Return all styles from the old version to be deleted
              return this.system.storage.find('Style',
                MatchExp.atom({ key: 'version', value: ['!=', newVersion] }),
                undefined,
                ['id']
              );
            }
    ...
    const styles1 = await system.storage.find('Style',
      undefined,
      undefined,
      ['id', 'content', 'status', 'version']
     );
      
    expect(styles1).toHaveLength(2);
    expect(styles1[0].status).toBe('published');
    expect(styles1[1].status).toBe('draft');
    expect(styles1[0].version).toBe(versionInfo1.version);
    expect(styles1[1].version).toBe(versionInfo1.version);
    
    // 这个时候 versioned style 和 style 应该是一致的，应为是取的 publish 之后的版本。
    const versionedStyles = await system.storage.find('VersionedStyle',
      undefined,
      undefined,
      ['id', 'content', 'status', 'version']
    );

    expect(versionedStyles).toHaveLength(2);
    expect(versionedStyles[0].status).toBe('published');
    expect(versionedStyles[1].status).toBe('draft');
    expect(versionedStyles[0].version).toBe(versionInfo1.version);
    expect(versionedStyles[1].version).toBe(versionInfo1.version);
    ...
    const stylesBeforeRollback = await system.storage.find('Style',
      undefined,
      undefined,
      ['id', 'content', 'version']
    );
    expect(stylesBeforeRollback).toHaveLength(3);
    expect(stylesBeforeRollback[0].content).toBe('Style 1 content');
    expect(stylesBeforeRollback[1].content).toBe('Style 2 content');
    expect(stylesBeforeRollback[2].content).toBe('Style 3 content');
    ...
    const stylesAfterRollback = await system.storage.find('Style',
      undefined,
      undefined,
      ['id', 'content', 'status', 'version']
    );

    expect(stylesAfterRollback).toHaveLength(2)
    expect(stylesAfterRollback[0].content).toBe('Style 1 content');
    expect(stylesAfterRollback[1].content).toBe('Style 2 content');
    expect(stylesAfterRollback[0].status).toBe('published');
    expect(stylesAfterRollback[1].status).toBe('draft');
    expect(stylesAfterRollback[0].version).toBe(versionInfo3.version);
    expect(stylesAfterRollback[1].version).toBe(versionInfo3.version);
