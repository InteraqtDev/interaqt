...
describe('Global data dependency', () => {
  test('should trigger computation when global data changes', async () => {
    ...
    // 设置初始全局值
    await system.storage.set('state', 'globalMultiplier', 10);
    ...
    // 更新全局值
    await system.storage.set('state', 'globalMultiplier', 15);
    ...
    // 再次更新全局值
    await system.storage.set('state', 'globalMultiplier', 20);
  ...
  test('should handle multiple global dependencies', async () => {
    ...
    // 设置初始全局值
    await system.storage.set('state', 'taxRate', 0.1); // 10% 税率
    await system.storage.set('state', 'discount', 0.2); // 20% 折扣
    ...
    // 更新税率
    await system.storage.set('state', 'taxRate', 0.15); // 15% 税率
    ...
    // 更新折扣
    await system.storage.set('state', 'discount', 0.3); // 30% 折扣
