...
describe('simple interaction', () => {
    ...
    beforeEach(async () => {
        ...
        const userARef = await system.entities.create('User', {name: 'A'})
        userAId = userARef.id

        const userBRef = await system.entities.create('User', {name: 'B', supervisor: userARef})
        userBId = userBRef.id

        const userCRef = await system.entities.create('User', {name: 'C', supervisor: userBRef})
        userCId = userCRef.id
    })
    ...
    test('simple leave request example test', async () => {
        ...
        const requests1 = await controller.system.entities.find('Request', undefined, undefined, ['*', ['from', {attributeQuery: ["*"]}]])
        ...
        const userA: User = await system.entities.findOne('User', BoolExp.atom({
                key: 'id',
                value: ['=', userAId]
            }), undefined, ['*'])

        const userB: User = await system.entities.findOne('User', BoolExp.atom({
                key: 'id',
                value: ['=', userBId]
            }), undefined, ['*'])
        ...
        const requests4 = await controller.system.entities.find(
            'Request',
            undefined,
            undefined,
            ['*']
